...
class User(db.Model):
    ...
    def authenticate(klass, nickname, password):
        ...
        logging.info("authenticate-- user:" + nickname + " password:" + password)
        
        user = klass.get_by_key_name(nickname)
        if user:
            n_salt = user.pwd.index('$')
            logging.info("user: %s password:%s suspended:%d, n_salt:%d" %(user.nickname,user.pwd,user.suspended,n_salt))
